...
{
    public class GetTeacherDataByPasswordCommand : BaseNetworkCommand
    {
        [Inject] public ChildModel ChildModel { get; private set; }

        private string apiPath = APIPaths.GET_TEACHER_BY_PASSWORD.ToDescription();
        private GetTeacherByPasswordRequestModel request;
        private ChildModel response;

        public override void Execute()
        {
            Retain();
            request = EventData.data as GetTeacherByPasswordRequestModel;
        ...
        private void Parse(JObject jsonObject)
        {
            ...
            {
                Debug.LogErrorFormat("GetTeacherDataByPasswordCommand parse error. Error text: {0}", e);

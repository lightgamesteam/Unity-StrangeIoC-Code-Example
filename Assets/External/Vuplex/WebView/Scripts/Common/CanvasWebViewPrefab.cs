...
namespace Vuplex.WebView {
    ...
    /// </remarks>
    /// <remarks>
    /// 3D WebView for iOS doesn't support video inside a Canvas with Render Mode 'Screen Space - Overlay'. If you need video support on iOS, please use 'Screen Space - Camera' instead.
    /// </remarks>
    ...
    public class CanvasWebViewPrefab : MonoBehaviour {
        ...
        [Header("Debugging")]
        /// <summary>
        /// Determines whether remote debugging is enabled using `Web.EnableRemoteDebugging()`.
        /// </summary>
        [Tooltip("Determines whether remote debugging is enabled using Web.EnableRemoteDebugging().")]
        ...
        void _init(WebViewOptions options, IWebView initializedWebView = null) {
            ...
            if (!_setCustomPointerInputDetector) {
                ...
                if (inputDetector == null) {
                    WebViewLogger.LogWarning("The CanvasWebViewPrefab instance has no CanvasPointerInputDetector, so pointer events will not be detected.");
                } else {
        ...
        void _instantiateWebViewPrefabIfNeeded() {
            ...
            if (_webViewPrefab == null) {
                ...
                _webViewPrefab.RemoteDebuggingEnabled = RemoteDebuggingEnabled;
            }
        }
        ...
        void _updateFieldsIfNeeded() {
            ...
            if (_webViewPrefab.LogConsoleMessages != LogConsoleMessages) {
                _webViewPrefab.LogConsoleMessages = LogConsoleMessages;
            }
            if (_webViewPrefab.RemoteDebuggingEnabled != RemoteDebuggingEnabled) {
                _webViewPrefab.RemoteDebuggingEnabled = RemoteDebuggingEnabled;
            }
        ...
        void WebViewPrefab_Initialized(object sender, EventArgs e) {
            ...
            if (InitialResolution <= 0) {
                WebViewLogger.LogWarningFormat("Invalid value for CanvasWebViewPrefab.InitialResolution: {0}. The resolution will instead default to 1.", InitialResolution);
                resolution = 1;
            }
            ...
            if (!String.IsNullOrEmpty(initialUrl)) {
                ...
                if (canvas != null && canvas.renderMode == RenderMode.ScreenSpaceOverlay) {
                    WebViewLogger.LogWarning("3D WebView for iOS doesn't support video inside a Canvas with Render Mode 'Screen Space - Overlay'. If you need video support on iOS, please use 'Screen Space - Camera' instead.");
                }
            #endif
        }
    }

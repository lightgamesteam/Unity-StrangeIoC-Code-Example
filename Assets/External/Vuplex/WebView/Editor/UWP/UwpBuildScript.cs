    ...
    public class UwpBuildScript : IPreprocessBuild {
        ...
        public void OnPreprocessBuild(BuildTarget buildTarget, string buildPath) {
            ...
            if (EditorUserBuildSettings.wsaUWPBuildType != WSAUWPBuildType.XAML) {
                var message = String.Format("Unsupported build type: 3D WebView for UWP requires a project Built Type of \"XAML Project\", but the built type is currently set to {0}. Please set the project build type to \"XAML Project\". For more information, see https://developer.vuplex.com/webview/getting-started#uwp .", EditorUserBuildSettings.wsaUWPBuildType);
                throw new BuildFailedException(message);
            }

            // Note: XRSettings.enabled is always false at build time.
            var mixedRealityEnabled = XRSettings.supportedDevices.Length > 0 && XRSettings.supportedDevices[0] == "WindowsMR";
        ...
        static void _copyRuntimeComponent(string pathToBuiltProject) {
            ...
            var destinationPluginsDirectoryPath = EditorUtils.PathCombine(new string[] { pathToBuiltProject, Application.productName, "Plugins" });
        ...
        static string _getRuntimeComponentPath() {
            ...
            var expectedPath = EditorUtils.PathCombine(new string[] { Application.dataPath, "Vuplex", "WebView", "Plugins", "UWP", WINDOWS_RUNTIME_COMPONENT_NAME });
            if (File.Exists(expectedPath)) {
                return expectedPath;
            }
            // The Windows Runtime component isn't in the default location (Assets/Vuplex/WebView/Plugins/UWP/{name}).
            ...
            var errorMessage = String.Format("Vuplex.WebView build error: unable to locate the {0} file in the Assets folder. It's not in the default location ({1}), and {2} instances of the directory were found in Assets folder.", WINDOWS_RUNTIME_COMPONENT_NAME, expectedPath, files.Count());
        ...
        static void _updateAppManifest(string pathToBuiltProject) {
            ...
            var filePath = EditorUtils.PathCombine(new string[] {  pathToBuiltProject, Application.productName, "Package.appxmanifest" });
            ...
            if (!packageContentsResult.Success) {
                throw new Exception(String.Format("Vuplex.WebView build error: the Visual Studio project at {0} has no <Package> element.", filePath));
            }
            ...
            if (applicationsElementResult.Success) {
                ...
                if (potentialSectionsToModify.Length > 1) {
...
";
            ...
            if (extensionsContentsResult.Success) {
                ...
                var extensionXml = String.Format("  <Extensions>{0}  </Extensions>\n", extensionElementXml);
        ...
        static void _updateVisualStudioProject(string pathToBuiltProject) {
            ...
            var filePath = EditorUtils.PathCombine(new string[] {  pathToBuiltProject, Application.productName, Application.productName + ".vcxproj" });
            var fileXml = File.ReadAllText(filePath, Encoding.UTF8);
            var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(WINDOWS_RUNTIME_COMPONENT_NAME);

            if (fileXml.Contains(String.Format("Include=\"{0}\"", fileNameWithoutExtension))) {
                // The Visual Studio project already has a reference to the Windows Runtime component.
                return;
            }
            var index = fileXml.IndexOf("</Project>");
            if (index == -1) {
                throw new Exception(String.Format("Vuplex.WebView build error: the Visual Studio project at {0} has no closing </Project> tag.", filePath));
            }
            // Match the file's expected indentation.
            var xmlTemplate = @"
  <ItemGroup>
